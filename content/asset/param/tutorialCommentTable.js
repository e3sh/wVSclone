...
//1玉　2B　3S　4L　5Coin 6Key 7_1UP 8Lamp 9map 10+ 11A 12B 13X 14Y
	...
	t[4] = ["「武器の強化」:武器の強化状態は、敵に拾われてしまうとリセットされる"
		,"また、手持ち武器以外の強化状態は強化には反映しない"
		,"(強化された同武器を後から入手しても +1 )"
		];//triger player.js

	t[5] = ["「レベルアップ」:経験値が一定に上がるとレベルアップ出来"
	...
	t[7] = ["「スピードアップ」:オプションが付く間は数に合わせて"
		,"自機の速度が向上する"
		]; //triger player.js

	t[8] = ["「オプション」:①が10個毎にオプションが1つ付き、"
		,"10個以下になると消滅します。(最大4つ)"
		]; //triger player.js
	...
	t[21] = ["「アイテム:⑦」:1UPアイテム。自機+1。EXP+100"
		];

	t[22] = ["「アイテム:⑥」:扉に入り次のステージへ行ける。EXP+10"
		];

	t[23] = ["「アイテム:②爆弾」:使用で範囲内の敵に一律のダメージを与える"
		,"ダメージはレベルアップで強化出来る。入手時EXP+8"
		];

	t[24] = ["「アイテム:③シールド」:使用で一定時間内、敵の攻撃を無効に出来る"
		,"効果時間はレベルアップで強化出来る。入手時EXP+8"
		];

	t[25] = ["「アイテム:④ライフ」:使用で一定値のHPの回復とHP最大値+1"
		,"回復値はレベルアップで強化出来る。入手時EXP+8"
		];

	t[26] = ["「アイテム:⑧ランプ」:敵の位置と鍵の場所が表示される"
		,"ステージ毎に入手。入手時EXP+10"
		];

	t[27] = ["「アイテム:⑨地図」:ステージの地図が表示される"
		,"ステージ毎に入手。入手時EXP+10"
		];

	t[35] = ["「アイテム:⑤」:ポイントアイテム。EXP+10"];
