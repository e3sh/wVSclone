 ...
 * @class
 * @classdesc
 * ステージクリア後の結果画面を管理するシーンです。<br>\
 * 次のステージへの進行を促し、アイテム取得ボーナスや<br>\
 * ステージクリア条件のチェックを行います。
 */
class sceneResult {
    /**
     ...
     * @description
     * `sceneResult`インスタンスを初期化します。<br>\
     * 「OK」ボタンを準備し、その位置と次のシーンへの遷移先を設定します。 
     */
    constructor(state) {
        ...
        /**
         * @description
         * 結果シーンの状態をリセットし、描画を準備します。<br>\
         * 背景描画を停止し、ランダムなアイテム取得ボーナスをダイアログに設定します。<br>\
         * ステージクリア条件のチェックも行います。 
         */
        function scene_reset() {
            ...
            /**
             * @returns {object} keyitemslist object
             * @description
             * プレイヤーが特定のキーアイテム（Amulet, Ring, Mirror）を所持しているかをチェックし、<br>\
             * その結果をオブジェクトとして返します。
             */
            function itemcheck() {
                ...
                let r = [];
                for (let i in KEYITEMS) {
                    r[i] = state.obCtrl.item[KEYITEMS[i]];
                    if (Boolean(r[i])) {
         ...
         * @description
         * 結果画面の入力処理と画面遷移ロジックです。<br>\
         * キーボード入力（Zキーなど）を検出し、<br>\
         * アイテム取得ボーナスを実行し、次のステージへの遷移を制御します。
        */
         ...
         * @description
         * 結果画面のUI要素を描画します。<br>\
         * ステージクリアのメッセージ、「OK」ボタン、<br>\
         * および取得アイテムのダイアログを表示します。
         */
        function scene_draw() {
         ...
         * @class
         * @classdesc
         * ステージクリア時のアイテム取得ボーナス選択肢を提供するダイアログ形式のUIを管理します。
         */
        class DialogControl {
            constructor(mlist) {
                 ...
                 * 選択されたオプションのテキストを「GET ...」に変更し、取得済みフラグを立てます。
                 */
                this.exec = function () {
                    for (let i in menulist) {
                        let m = menulist[i];
