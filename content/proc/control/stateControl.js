 ...
 * @class
 * @classdesc
 * ゲーム全体の状態を統合的に管理する中核クラスです。<br>\
 * 定数、設定、結果、システム、ゲーム進行、データベースなどの<br>\
 * 全ての主要な状態コンポーネントを保持し、アクセスポイントとして機能します。
 */
class stateControl {
	/**
	 * @constructor
	 ...
	 * @description
	 * `stateControl`インスタンスを初期化します。<br>\
	 * `ConstantData`、`stateConfig`、`stateResult`、`stateSystem`、`stateGame`など、<br>\
	 * ゲームの様々な状態を管理するサブクラスのインスタンスを生成し、保持します。 
	 */
	constructor(g) {
