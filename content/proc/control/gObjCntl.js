 ...
 * @class
 * @classdesc
 * ゲーム内の全てのオブジェクトを統括管理します。<br>\
 * オブジェクトの生成、移動、メッセージ処理、および衝突判定を行い、<br>\
 * オブジェクトの描画優先順位制御も担当し、描画を最適化します。
 */
class gObjectControl {
    /**
     * @constructor
     * @param {Gamecore.state} state GameCore.state
     * @description
     * `gObjectControl`のインスタンスを初期化します。
     * オブジェクトリスト、衝突判定ツリー（`CLinear4TreeManager`）、
     * アイテム所持状況、天井表示フラグ、メッセージログなどを準備します。
     */
    constructor(state) {
        ...
        const SP = state.Constant.layer.SP;
        let scrn = dev.graphics[SP];
        ...
        /**
         * 
         * @param {string} name gObjectClass.name 
         * @returns {boolean}
         * @description
         * 指定された名前のオブジェクトが現在アクティブなオブジェクトリストに存在するかをチェックします。<br>\
         * プレイヤー（"mayura"）の生存確認などに使用されます。
         */        
        ...
        /**
         * @description
         * キャラクター表示の個別重ね合わせ制御（Y座標ソート）を行うための内部ユーティリティです。<br>\
         * スプライトが奥から手前へ正しく描画されるようにソートします。
         */
        ...
        let itemtable = state.Database.chrItemtable;
        ...
        const WEAPONS = state.Constant.item.WEAPONS;
        const USEBLE = state.Constant.item.USEBLE;
         ...
         * @param {*} cont_flag 
         * @description
         * オブジェクト管理システムをリセットします。<br>\
         * 衝突判定ツリーの再初期化、オブジェクトリストのクリア、<br>\
         * アイテムやスコアのリセットなどを行います。
         * @todo
         * アイテムも管理しているため、Resetモードが煩雑<br>\
         * オブジェクトのみに変更するべき
         */
        this.reset = function (cont_flag) {
            ...
            if (!cdt.init(6, 0, 0, 3000, 3000)) { console.error("CL4TMgrFnc init Failed!"); }

            restart_count = 0;

            if (state.Game.cold) {
                this.score = 0; //経験値として使用
        ...
        /**
         * @method
         * @param {mapScenarioControl} mapsc マップ管理オブジェクト 
         * @param {GameMainInputParam} input MainTaskで検出したキー入力状態の纏めオブジェクト
         * @description
         * 全てのゲームオブジェクトの移動処理と、各オブジェクトが発行したメッセージの処理を実行します。<br>\
         * オブジェクトの画面外判定、ワールド境界での座標変換、<br>\
         * そして衝突判定ツリーへの登録も行います。
         */
         ...
         * @method
         * @description
         * マップ状態の再起動指示(ゲームオブジェクトはそのまま)
         * 自機DEAD後のリスタートでGameSceneから呼ばれる
         * (RESETはされない)
         */ 
        ...
        /**
         * @method
         * @param {dev.graphics} wscreen dev.graphics[SP or FG] 
         * @param {boolean} mode prioritySurface mode select /gobjclass instance select 
         * @description
         * 全てのゲームオブジェクトの描画処理を実行します。<br>\
         * オブジェクトの可視性と描画優先順位に基づいてソートし、<br>\
         * 適切なスクリーンに描画します。デバッグモード時の枠表示もここで行われます。
         */
        ...
        /**
         * @method
         * @param {dev.graphics} wscreen dev.graphics[BG]
         * @description
         * 背景レイヤー（BG面）にキャラクター用の影を描画します。<br>\
         * 弾やエフェクト以外の、影を持つべきオブジェクトに適用されます。
         */
         ...
         * @param {number | string} sc シナリオ
         * @param {number | string} id ID
         * @param {gObjectClass} parent 親オブジェクト
         * @description
         * 新しい`gObjectClass`インスタンスを生成し、ゲームオブジェクトリストに登録します。<br>\
         * 位置、位置、キャラクター番号、シナリオ、ID、親オブジェクトなどを設定します。
         */
         ...
         * @returns オブジェクト数
         * @description
         * 現在オブジェクトリストに格納されているオブジェクトの総数を返します。<br>\
         * （削除済みのオブジェクトも含む、配列の長さを返します。）
         */
        this.num = function () {
        ...
        /**
         * @method
         * @returns アクティブオブジェクト数
         * @description
         * 現在オブジェクトリストに格納されている、生存状態（living）のオブジェクトの数をカウントして返します。
         */
        this.cnt = function () {
        ...
        /**
         * 
         * @param {dev.graphics} scrn 表示するScreen
         * @param {gObjectClass} o gObjectClassインスタンス
         * @description
         * オブジェクトのモーションパターンとアニメーションフレームに基づいてスプライトを描画します。<br>\
         * モーションの更新間隔と現在のフレームを管理します。
         */
         ...
         * @param {number} x 座標
         * @param {number} y 座標
         * @param {number} mp モーションパターン
         * @param {number} mpcnt アニメーションフレーム
         * @param {number} r 回転方向
         * @param {number} alpha 透明度
         * @param {number} size 拡大倍率
         * @description
         * 指定されたモーションパターン、アニメーションフレーム、回転、透明度、サイズで、<br>\
         * スプライトを画面に描画します。<br>\
         * 画像データとパターン情報を組み合わせて描画を制御します。
         */
        ...
        /**
         * @method
         * @param {number} num キャラクター番号(アイテム)
         * @description
         * ゲームシーン以外の場所からアイテムを追加するためのメソッドです。<br>\
         * 定されたアイテム番号の所持数を増やし、必要な場合はアイテムスタックにも追加します。
         * @example
         * 面クリアのリザルト報酬追加の処理の為の呼び出しメソッド
         * @todo
         * アイテム管理が外にあれば不要となる
         */
        this.get_item = function (num) {
            ...
            if (WEAPONS.includes(num)){
                //((num >= 15) && (num <= 19)) || (num == 50)) {
        ...
        /**
         * @method
         * @param {number} キャラクター番号
         * @returns {object} スプライトデータ 
         * @description
         * キャラクター番号から、そのキャラクターのモーションパターンの、<br>\
         * 最初のフレームのスプライトデータを返します。
         */
